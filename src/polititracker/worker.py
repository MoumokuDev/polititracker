...
log = logging.getLogger("polititracker.worker")
...
def index_daily() -> None:
    from polititracker.search import indexer  # heavy import (torch), deferred

    _job(lambda s: indexer.run(s))


def topics_daily() -> None:
    from polititracker.search import topics  # heavy import (torch), deferred
